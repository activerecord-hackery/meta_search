...
module MetaSearch
  module Translate
    def self.word(key, options = {})
      I18n.translate(:"meta_search.#{key}", :default => key.to_s)
    end

    def self.predicate(key, options = {})
      I18n.translate(:"meta_search.predicates.#{key}", :default => key.to_s)
    end
    ...
    def self.attribute(key, options = {})
      ...
      predicate = MetaSearch::Configuration.predicate_keys.detect {|p| original_name.match(/_#{p}$/)}
      attributes_str = original_name.sub(/_#{predicate}$/, '')
      attribute_names = attributes_str.split(/_and_|_or_/)
      combinator = attributes_str.match(/_and_/) ? :and : :or
      defaults = base_ancestors.map do |klass|
        :"meta_search.attributes.#{klass.model_name.underscore}.#{original_name}"
      end
    ...
    def self.association(key, options = {})
      ...
      defaults = key.blank? ? [:"#{context.klass.i18n_scope}.models.#{context.klass.model_name.underscore}"] : [:"meta_search.associations.#{context.klass.model_name.underscore}.#{key}"]
    ...
    def self.attribute_name(context, name, include_associations = nil)
      ...
      interpolations[:attr_fallback_name] = I18n.translate(
        (associated_class ?
          :"meta_search.attributes.#{associated_class.model_name.underscore}.#{attr_name}" :
          :"meta_search.attributes.#{context.klass.model_name.underscore}.#{attr_name}"
        ),
        :default => [
          (associated_class ?
      ...
      defaults = [
        :"meta_search.attributes.#{context.klass.model_name.underscore}.#{name}"
      ]
