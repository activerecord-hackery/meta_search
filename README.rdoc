...
In your view:
  ...
  <% form_for :search, @search, :html => {:method => :get} do |f| %>
    ...
    <%= f.label :comments_created_on_greater_than, 'With comments after' %>
    <%= f.datetime_select :comments_created_on_greater_than, :include_blank => true %><br />
    <!-- etc... -->
    <%= f.submit %>
  <% end %>
  
The default Where types are listed at MetaSearch::Where.
...
create an initializer (<tt>/config/initializers/meta_search.rb</tt>, for instance):

  MetaSearch::Where.add :between, :btw, {:conditional => 'BETWEEN', :substitutions => '? AND ?'}
...
MetaSearch adds a helper for this:
  ...
  <%= f.multiparameter_field :moderations_value_between,
      [{:field_type => :text_field}, {:field_type => :text_field}] %>

<tt>multiparameter_field</tt> works pretty much like the other FormBuilder helpers, but this
version lets you sandwich an array of fields between the attribute and the usual options hash.
See MetaSearch::Helpers::FormBuilder for more info.
