...
This version of MetaSearch depends on some functionality I've added to Arel which
has not yet been rolled into the official version. You can still try it out,
though, by adding these lines to your Gemfile:
...
then supply the <tt>meta_sort</tt> parameter in your form. The parameter takes the form
<tt>column.direction</tt> where +column+ is the column name, and +direction+ is one of
